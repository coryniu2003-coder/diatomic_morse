...
1. Activate `mace-env`.
2. Point `mace-train` at `ar_train.xyz`/`ar_val.xyz` using the `--model_init` flag referencing the public MP-0b3 checkpoint.
3. After convergence, export the fine-tuned model and convert it to a TorchScript file for the LAMMPS deck.

See the main workflow notes from Codex for the exact commands when you're ready to launch training.
